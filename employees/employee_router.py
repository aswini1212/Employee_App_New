...
from employees.schemas import AddressResponse, AssociationResponse, EmployeeCreate, EmployeeResponse, EmployeeResponseId, EmployeeUpdate, AddressUpdate
...
async def fetch_all(db:AsyncSession=Depends(get_db),dependencies=[Depends(require_role(EmployeeRole.HR))]):
    result=await employee_service.fetch_all(db)
    return result

#getting an employee with id
@router.get("/id/{emp_id}",tags=["Employees"],response_model=EmployeeResponse,dependencies=[Depends(require_role(EmployeeRole.HR))])
async def fetch_one(emp_id:int, db:AsyncSession= Depends(get_db),_current_user: TokenPayload=Depends(get_current_user)):
...
@router.put("/{emp_id}/addresses/{address_id}",tags=["Employees-Address"],response_model=AddressResponse)
async def update_empaddress(body: AddressUpdate,emp_id:int,address_id:int,db:AsyncSession=Depends(get_db)):
    address=await employee_service.update_empaddress(body,emp_id,address_id,db)
    return address

#deleting an employee with id            
@router.delete("/{emp_id}",tags=["Employees"],response_model=EmployeeResponse,dependencies=[Depends(require_role(EmployeeRole.HR))])
...
@router.delete("/{emp_id}/adresses/{address_id}",tags=["Employees-Address"],response_model=AddressResponse)
async def remove_empaddress(emp_id:int,address_id:int,db: AsyncSession=Depends(get_db)):
...
@router.post("/{emp_id}/departments/{department_id}",tags=["Employee-Department Association"],response_model=AssociationResponse)
async def create_empdept(emp_id:int,department_id:int, db:AsyncSession=Depends(get_db)):
...
@router.delete("/{emp_id}/departments/{department_id}",tags=["Employee-Department Association"],response_model=AssociationResponse)
async def detach_empdept(emp_id:int,department_id:int,db:AsyncSession=Depends(get_db)):
    association= await employee_service.create_empdept(emp_id,department_id,db)
    return association
