...
#to overcome the problem of greenlet error caused due to lazy loading we areexplicitly loading the employees with addresses again
    stmt = (select(Employee).options(selectinload(Employee.addresses)).where(Employee.id == db_employee.id))
...
async def fetch_one(emp_id:int,db:AsyncSession):
    stmt=select(Employee).options(selectinload(Employee.addresses)).where(Employee.deleted_at.is_(None)).where(Employee.id==emp_id)
    db_employee = await db.scalars(stmt)
...
async def get_by_name(name:str,db:AsyncSession):
    stmt=select(Employee).options(selectinload(Employee.addresses)).where(Employee.deleted_at.is_(None)).where(Employee.name.like(f"%{name}%"))
...
async def get_by_email(email:str,db:AsyncSession) -> Employee | None:
    stmt=select(Employee).options(selectinload(Employee.addresses)).where(Employee.email==email).where(Employee.deleted_at.is_(None))
    result = await db.scalars(stmt)
    db_employee=result.first()
    if not db_employee:
        raise NotFoundException(detail="Employee not found")
...
async def update(emp_id:int,db:AsyncSession,name:str, email:str,age:int,password:str):
...
async def update_empaddress(body:AddressUpdate,emp_id:int,address_id:int,db:AsyncSession):
    stmt = (
        select(Address)
...
async def remove_empaddress(emp_id,address_id,db):
    ...
    address = next(
        (addr for addr in db_employee.addresses if addr.id == address_id and addr.deleted_at is None),None)
...
async def detach_empdept(emp_id:int,department_id:int,db:AsyncSession):
    stmt = (select(Emp_Dept_Assoc).where(Emp_Dept_Assoc.employee_id == emp_id).where(Emp_Dept_Assoc.department_id == department_id).where(Emp_Dept_Assoc.deleted_at.is_(None)))
    result = await db.scalars(stmt)
